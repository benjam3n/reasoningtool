...
| `/ar` | assume right | You believe something is right and want to know what it commits you to | Deep recursive rightness search. Traces implications, foreclosures, and costs of a claim being true. |
| `/araw` | assume right assume wrong | You have a specific claim but don't know if it's right or wrong | For every claim, explore what follows if true (AR) and what breaks if false (AW). Tests both sides with equal rigor. |
...
| `/aw` | assume wrong | You suspect something is wrong and want to find exactly why | Deep recursive wrongness search. Finds failure reasons, derives alternatives from what breaks. |
...
| `/u` | universalize | You want a quick map of possibilities you haven't considered | Extract the complete space of assumptions, dimensions, and alternatives from any claim. |
| `/uaua` | universalize araw universalize araw | You don't know what the options are and need thorough analysis | Full space mapping + rigorous testing. Maps the space, tests candidates, finds edge cases, validates survivors. |
