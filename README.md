...
/dcp should I take this job offer
```
...
| `/ar` | assume right | Assume Right - Deep recursive rightness search. |
| `/araw` | assume right assume wrong | Assume Right / Assume Wrong search - For every claim, explore what follows if true (AR) and what breaks if false (AW). |
...
| `/aw` | assume wrong | Assume Wrong - Deep recursive wrongness search. |
...
| `/foht` | figure out how to | Figure Out How To - Method discovery when you know WHAT but not HOW. |
| `/fowwr` | figure out what went wrong | Figure Out What Went Wrong - Backward causal diagnosis of failures. |
...
| `/p` | propose | Convert analysis findings into steelmanned, actionable plans with conditional recommendations. |
...
| `/qo` | question order | Find the satisfying unresolved question that opens a document and order all questions in a dependency chain. |
...
| `/sbfow` | still bad figure out why | Still Bad, Figure Out Why - Self-diagnosis when output was rejected. |
...
| `/sf` | save file | Save the output from the most recent skill invocation to the appropriate library folder. |
...
| `/u` | universalize | Universalize - Extract the complete space of assumptions, dimensions, and alternatives from any claim. |
| `/uaua` | universalize araw universalize araw | UAUA Combined Exploration - Universalize, ARAW, loop, synthesize. |
...
| `/w` | write | Generative writing tool. |
...
| `/wt` | want to | Want To - AR-based goal exploration. |
